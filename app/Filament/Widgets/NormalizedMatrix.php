...
class NormalizedMatrix extends BaseWidget
{
    protected function getNormalizedValues(): array
    {
        ...
        foreach ($alternatives as $alternative) {
            foreach ($criterias as $criteria) {
                ...
                $values[$alternative->id][$criteria->id] = $normalized;
            }
        }
    ...
    {
        $normalized = $this->getNormalizedValues();
        $criterias = Criteria::all();
    ...
    {
        return $table
            ->heading('Matriks Ternormalisasi (R)')
